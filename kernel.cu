...
int main()
{
	...
	/************场景参数设置***********/
	const double c = 3e8;		//波的传播速度，单位：m/s
	const double f0 = 75e9;		// 雷达发射信号中心频率，单位：Hz
	const double B = 35e9;		// 带宽，单位：Hz
	...
	double delta_f = B / (Nf - 1);		//收发机频率采样间隔，单位：Hz
	double rhoz = c / 2 / B;			//距离维理论分辨率
	double TRZ = -R0;					//收发机距离维位置，单位：m
...
//	memcpy(z_map.pointer, Kz.pointer, Kz.length*sizeof(double));
	...
	*/
	...
	for (index_y = 0; index_y < Ny; index_y++)
	{
		...
		for (index_x = 0; index_x < Nx; index_x++)
		{
			//对应语句[x3,z3]=find(K4_uni(j,:)>K3_uni(j,Nf));
			pms_clear_dpmscl(&z3);
