/*
*PMS is short for pointer manage struct
*pms structure is designed for matrix
*Record the matrix pointer, length, number of dimensions and the length of each dimension
*Complete with matrix multiplication, dimension conversion and other functions
*You can operate directly on the pms structure
*Avoid manual manipulation of each matrix dimension information
*/
#include <stdio.h>
#include <stdlib.h>
#include <memory.h>
...
/********************
Macro function: Offset address calculation
...
/********************
Macro definition function: Error type
...
/**************
Define the matrix pointer management structure
*************/
...
/***************************************
The function pms_set_parameter is used to set the parameters of the pms structure
The call format is
pms_set_parameter(&pms,x);
pms_set_parameter(&pms,x,y);
pms_set_parameter(&pms,x,y,z);
Write a few parameters if you have a few.
*****************************************/
...
/***************************************
The pms_creat_memory function is used to request memory
The call format is
pms_creat_memory(&pms);
Please make sure that the parameters are set in pms
*****************************************/
...
#endif
/***************************************
The pms_creat_memory_with_paramete function can do both allocating parameters and requesting memory
The call format is
pms_creat_memory_with_paramete(&pms,x);
pms_creat_memory_with_paramete(&pms,x,y);
pms_creat_memory_with_paramete(&pms,x,y,z);
Make sure the parameters are set in pms.
*****************************************/
...
/***************************
Corresponds to the linspace function in MATLAB
start is the starting value
end is the end value
num takes the number of points
out is the array pointer
order for the order (positive or inverted order)
*****************************/
...
*********************************/
void matmul(double* a, double* b, double* c, int n, int m, int t)		//Input parameters: c = a*b, m - number of rows, n - number of columns
{
	int i, j, k;
	for (i = 0; i < n; i++)
	{
...
void pms_matmul(_dpms* out, _dpms* in1, _dpms* in2)
{
	...
	{
		printf("The matrix multiplication is incorrect");
...
{
	if (pms_rsc->length == pms_dst->length)		//If they are equal in length, then
	{
...
void pmscl_enable_area_init(_dpmscl* dpms)
{
	...
	case 3:
		...
		break;
	default:
		printf("Dimensional invalidation of pms");
